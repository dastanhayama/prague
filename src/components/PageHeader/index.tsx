...
const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  const wrapperClass =
    `flex  pb-2xl relative  pt-xl gap-y-xl desktop:gap-0 ${banner ? 'desktop:px-8xl px-xl' : 'px-xl'} ${centered ? 'flex-col items-center justify-center gap-y-xl desktop:gap-y-2xl' : 'desktop:flex-row flex-col items-start justify-between'}`
  ...
  const labelClass = `desktop:text-[30px] desktop:leading-[38px] text-[24px] leading-[32px] font-semibold text-primary-900 ${centered ? 'text-center': 'text-left'}`
  const supportingTextClass = `text-[16px] leading-[24px] text-tertiary-600 ${centered ? 'text-center' : 'text-left'} `

  const rightContainerClass =
    ` gap-x-lg gap-y-xl w-full desktop:w-auto ${centered ? 'flex flex-col justify-center items-center': 'flex desktop:flex-row flex-col items-start'}`
  const buttonGroupClass = `flex items-start gap-lg ${centered ? 'w-full desktop:w-auto' : ''}`
  ...
  const buttonSecondaryAltClass = `text-[#344054] bg-primary border border-[#D0D5DD] shadow-xs ${centered ? 'w-full desktop:w-auto' : 'w-auto'}`
  const buttonPrimaryClass = `bg-[#101828] border border-[#101828] text-primary ${centered ? 'w-full desktop:w-auto' : 'w-auto'}`
