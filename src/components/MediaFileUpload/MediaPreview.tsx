...
const LoaderIcon = ({ onCancel }: { onCancel: () => void }) => (
    ...
    <path d="M12 2C6.47715 2 2 6.47715 2 12" stroke="#344054" strokeWidth="4" strokeLinecap="round" />
      ...
      <button 
        ...
        className="absolute flex items-center justify-center bg-white rounded-full w-auto h-auto"
      >
...
const overlay_class = "absolute top-0 left-0 w-full h-full bg-black opacity-0 group-hover:opacity-20 group-active:opacity-30 rounded-[12px] pointer-events-none"
...
export default function MediaPreview({ files, onRemoveFile, disabled = false }: MediaPreviewProps) {
        ...
        >
          {file.status === 'loading' ? (
            <div className={loader_container_class}>
              <LoaderIcon onCancel={cancelUpload}/>
            </div>
          ) : (
            <div
